...
namespace RRTstar_planner
{
  ...
  void RRTstarPlannerROS::initialize(std::string name, costmap_2d::Costmap2DROS* costmap_ros)
  {
    ...
    if (!initialized_)
    {
      ...
      pubPathNodes = private_nh.advertise<visualization_msgs::MarkerArray>("path_point", 1);
    }
    else
  ...
  void RRTstarPlannerROS::publishPlan(const std::vector<geometry_msgs::PoseStamped>& path) {
    if (!initialized_) {
        ROS_ERROR(
